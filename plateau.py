...
class Plateau:
    ...
    def est_gagnant(self, pion):
        ...
        if self.cases[0,0].est_pion(pion) and self.cases[0,1].est_pion(pion) and self.cases[0,2].est_pion(pion):
            return True
        if self.cases[1,0].est_pion(pion) and self.cases[1,1].est_pion(pion) and self.cases[1,2].est_pion(pion):
            return True
        if self.cases[2,0].est_pion(pion) and self.cases[2,1].est_pion(pion) and self.cases[2,2].est_pion(pion):
            return True
        #check lines horizontale
        if self.cases[0,0].est_pion(pion) and self.cases[1,0].est_pion(pion) and self.cases[2,0].est_pion(pion):
            return True
        if self.cases[0,1].est_pion(pion) and self.cases[1,1].est_pion(pion) and self.cases[2,1].est_pion(pion):
            return True
        if self.cases[0,2].est_pion(pion) and self.cases[1,2].est_pion(pion) and self.cases[2,2].est_pion(pion):
            return True
         #test diagonals
        if self.cases[0,0].est_pion(pion) and self.cases[1,1].est_pion(pion) and self.cases[2,2].est_pion(pion):
            return True
        if self.cases[2,0].est_pion(pion) and self.cases[1,1].est_pion(pion) and self.cases[0,2].est_pion(pion):
            return True
        return False
    ...
    def choisir_prochaine_case(self, pion):
        ...
        assert pion in ["O", "X"], "Plateau: pion doit être 'O' ou 'X'."
        # check lines vertical
        if self.cases[0, 0].est_pion(pion) and self.cases[0, 1].est_pion(pion) and self.cases[0, 2].est_vide():
            return 0,2
        if self.cases[1, 0].est_pion(pion) and self.cases[1, 1].est_pion(pion) and self.cases[1, 2].est_vide():
            return 1, 2
        if self.cases[2, 0].est_pion(pion) and self.cases[2, 1].est_pion(pion) and self.cases[2, 2].est_vide():
            return 2, 2

        if self.cases[0, 0].est_pion(pion) and self.cases[0, 2].est_pion(pion) and self.cases[0, 1].est_vide():
            return 0, 1
        if self.cases[1, 0].est_pion(pion) and self.cases[1, 2].est_pion(pion) and self.cases[1, 1].est_vide():
            return 1, 1
        if self.cases[2, 0].est_pion(pion) and self.cases[2, 2].est_pion(pion) and self.cases[2, 1].est_vide():
            return 2, 1


        if self.cases[0, 1].est_pion(pion) and self.cases[0, 2].est_pion(pion) and self.cases[0, 0].est_vide():
            return 0, 0
        if self.cases[1, 1].est_pion(pion) and self.cases[1, 2].est_pion(pion) and self.cases[1, 0].est_vide():
            return 1, 0
        if self.cases[2, 1].est_pion(pion) and self.cases[2, 2].est_pion(pion) and self.cases[2, 0].est_vide():
            return 2, 0

        # check lines horizontale
        if self.cases[0, 0].est_pion(pion) and self.cases[1, 0].est_pion(pion) and self.cases[2, 0].est_vide():
            return 2, 0
        if self.cases[0, 1].est_pion(pion) and self.cases[1, 1].est_pion(pion) and self.cases[2, 1].est_vide():
            return 2, 1
        if self.cases[0, 2].est_pion(pion) and self.cases[1, 2].est_pion(pion) and self.cases[2, 2].est_vide():
            return 2, 2

        if self.cases[0, 0].est_pion(pion) and self.cases[1, 0].est_vide() and self.cases[2, 0].est_pion(pion):
            return 1, 0
        if self.cases[0, 1].est_pion(pion) and self.cases[1, 1].est_vide() and self.cases[2, 1].est_pion(pion):
            return 1, 1
        if self.cases[0, 2].est_pion(pion) and self.cases[1, 2].est_vide() and self.cases[2, 2].est_pion(pion):
            return 1, 2

        if self.cases[0, 0].est_vide() and self.cases[1, 0].est_pion(pion) and self.cases[2, 0].est_pion(pion):
            return 0, 0
        if self.cases[0, 1].est_vide() and self.cases[1, 1].est_pion(pion) and self.cases[2, 1].est_pion(pion):
            return 0, 1
        if self.cases[0, 2].est_vide() and self.cases[1, 2].est_pion(pion) and self.cases[2, 2].est_pion(pion):
            return 0, 2
        # test diagonals
        if self.cases[0, 0].est_pion(pion) and self.cases[1, 1].est_pion(pion) and self.cases[2, 2].est_vide():
            return 2, 2
        if self.cases[2, 0].est_pion(pion) and self.cases[1, 1].est_pion(pion) and self.cases[0, 2].est_vide():
            return 0, 2

        if self.cases[0, 0].est_pion(pion) and self.cases[1, 1].est_vide() and self.cases[2, 2].est_pion(pion):
            return 1, 1
        if self.cases[2, 0].est_pion(pion) and self.cases[1, 1].est_vide() and self.cases[0, 2].est_pion(pion):
            return 1, 1

        if self.cases[0, 0].est_vide() and self.cases[1, 1].est_pion(pion) and self.cases[2, 2].est_pion(pion):
            return 0, 0
        if self.cases[2, 0].est_vide() and self.cases[1, 1].est_pion(pion) and self.cases[0, 2].est_pion(pion):
            return 2, 0

        ligne = randrange(0,3)
        col = randrange(0,3)
        while (not self.position_valide(ligne, col)):
            ligne = randrange(0,3)
            col = randrange(0,3)
        return ligne, col